...
const accounts = [
  ['Ban giám đốc', 'director@vnpro.vn'], ['Kinh doanh', 'sales@vnpro.vn'], ['Kế toán', 'accounting@vnpro.vn'],
  ['Kho', 'warehouse@vnpro.vn'], ['Kỹ thuật', 'technical@vnpro.vn'], ['Quản trị', 'admin@vnpro.vn']
]

export function LoginPage() {
  const { user, login } = useAuth()
  const [email, setEmail] = useState('director@vnpro.vn')
  ...
  return <div className="login-page"><section className="login-hero"><div className="hero-logo"><Scale size={32}/><span>VNPRO Enterprise Hub</span></div><h1>Một nơi để điều hành toàn bộ doanh nghiệp.</h1><p>Theo dõi bán hàng, chi phí, dự án kỹ thuật, kho, công nợ và chăm sóc khách hàng trên cùng một nền tảng.</p><div className="hero-points"><div><ShieldCheck/> Vận hành theo phòng ban</div><div><LockKeyhole/> Audit log và kiểm soát dữ liệu</div></div></section>
